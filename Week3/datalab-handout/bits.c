...
int allOddBits(int x) {
    int compared = 0xAA | (0xAA << 8) | (0xAA << 16) | (0xAA << 24);
  return !((x & compared) ^ compared);
}
/* 
 * negate - return -x 
 *   Example: negate(1) = -1.
...
int isLessOrEqual(int x, int y) {
    ...
    int addition = (~x + 1) + y;
    int sign = addition >> 31 & 1;

    int tMin = 1 << 31;
    int xLeft = tMin & x;
    int yLeft = tMin & y;
    int result = xLeft ^ yLeft;
    result = (result >> 31) & 1;

    return (result & (xLeft >> 31) | (!sign & !result));
}
//4
/* 
